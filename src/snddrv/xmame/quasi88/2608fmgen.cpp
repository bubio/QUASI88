...
static void fmgen2608_stream_update(void *param, stream_sample_t **inputs,
                                    stream_sample_t **buffer, int length) {
...
static void *fmgen2608_start(int sndindex, int clock, const void *config) {
  ...
  info->stream =
      stream_create(0, 2, Machine->sample_rate, info, fmgen2608_stream_update);

  info->opna = new FM::OPNA;

  if (verbose_proc)
    printf("  OPNA Init with ROM DIR: %s\n",
           osd_dir_rom() ? osd_dir_rom() : "(null)");
  if (info->opna->Init(clock,
                       Machine->sample_rate ? Machine->sample_rate : 44100, 0,
                       osd_dir_rom())) {
    if (sound2_adpcm) {
...
READ8_HANDLER(FMGEN2608_status_port_0_A_r) {
  struct fmgen2608_info *info =
      (struct fmgen2608_info *)sndti_token(SOUND_FMGEN2608, 0);
...
READ8_HANDLER(FMGEN2608_status_port_1_A_r) {
  struct fmgen2608_info *info =
      (struct fmgen2608_info *)sndti_token(SOUND_FMGEN2608, 1);
  return info->opna->ReadStatus();
}
