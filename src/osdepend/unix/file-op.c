      ...
      for (i = 0; i < MAX_STREAM; i++) {
        if (osd_stream[i].fp) {
          if (osd_stream[i].sb.st_dev == sb.st_dev &&
            ...
            if (type == FTYPE_DISK && osd_stream[i].type == type &&
...
int osd_file_config_init(void) {
  ...
  if (!dir_cwd || !dir_rom || !dir_disk || !dir_tape || !dir_snap ||
      !dir_state || !dir_g_cfg || !dir_l_cfg) {
    return FALSE;
  }
